...
public class Node {
  ...
  */
  ...
  public void onCommand(String command) {
    ...
    if (this.matchMultiTransferCommand(command)) {
      return;
    }

    if (this.matchMultiServerCommand(command)) {
      return;
    }
  ...
  private boolean matchServerCommand(String command) {
    ...
    private boolean matchMultiTransferCommand(String command) {
    // multitransfer command syntax:
    //     multitransfer dest port first_localPort connection_count amount [interval sz]
    // Synopsis:
    //     Connect to a transfer server listening on port <port> at node
    //     <dest>, using local ports <first_localPort> through <first_localPort> +
    //      connection_count, and transfer <amount> bytes through each connection
    // Required arguments:
    //     dest: address of destination node
    //     port: destination port
    //     first_localPort: local port of first socket
    //     amount: number of bytes to transfer through each socket
    // Optional arguments:
    //     interval: execution interval of the transfer client, default 1 second
    //     sz: buffer size of the transfer client, default 65536
    String[] args = command.split(" ");
    if (args.length < 6 || args.length > 8 || !args[0].equals("multitransfer")) {
      return false;
    }
    ...
    try {
      int destAddr = Integer.parseInt(args[1]);
      int port = Integer.parseInt(args[2]);
      int first_localPort = Integer.parseInt(args[3]);
      int connection_count = Integer.parseInt(args[4]);
      ...
      TCPSock[] socks = new TCPSock[connection_count];

      for (int i = 0; i < connection_count; i++) {
        socks[i] = this.tcpMan.socket();
        socks[i].bind(first_localPort + i);
        int ret = socks[i].connect(destAddr, port);

        if (ret == -1)
          return false;
      }

      MichaelTestClient client = new
          MichaelTestClient(manager, this, socks, amount, interval, sz);
      client.start();
  ...
  private boolean matchMultiServerCommand(String command) {
    // multiserver command syntax:
    ...
    //     sz: buffer size of the transfer worker, default 65536
    String[] args = command.split(" ");
    if (args.length < 3 || args.length > 6 || !args[0].equals("multiserver")) {
      return false;
    }
    ...
    try {
      ...
      MichaelTestServer server = new
         MichaelTestServer(manager, this, sock, servint, workint, sz);
      server.start();
      logOutput("server started, port = " + port);
